...
class GoogleAdsClient {
  ...
  async initialize() {
    try {
      ...
      // Initialize Google Ads API client with OAuth app credentials only
  ...
  getCustomer(customerId) {
    ...
    // Authenticate per-customer with refresh token and MCC as login_customer_id
    const options = {
      refresh_token: config.googleAds.refreshToken,
      login_customer_id: config.googleAds.mccCustomerId
    };
    return this.client.Customer(String(customerId), options);
  }
